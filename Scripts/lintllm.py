...
Please do not output medium processes or other content.'''     
    ...
    rptfile = pri_loc+'/result/'+modelname+'res/'+moduleName+'_'+modelname+'.rpt'
...
def llmF(m,n,new_project,pri_loc):
    ...
    for k in range(m,n):
        ...
        print(formatted_time1+' '+moduleName+' '+modelname+'res Lint begin',flush=True)
        llmFun(moduleName,pri_loc)
        formatted_time2 = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        print(formatted_time2+' '+moduleName+' '+modelname+'res Lint end',flush=True)
    ...
    print('--->'+str(totalSeconds)+'s '+new_project+' '+modelname+'res Lint Time',flush=True)
def main():
    m = 1 # update
    n = 31 # update
    ...
    for project,folder in zip(new_project,folderlist):
        pri_loc = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Benchmark', folder)
